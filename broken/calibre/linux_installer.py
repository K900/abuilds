...
def get_url(fname):
    ...
    return 'http://sourceforge.net/projects/calibre/files/0.8.12/'+fname
...
class Reporter:
    ...
    def __init__(self, fname):
        try:
            self.pb  = ProgressBar(TerminalController(sys.stdout), 'Downloading '+fname)
        except ValueError:
...
def download_tarball():
    fname = 'calibre-0.8.12-i686.tar.bz2'
    if is64bit():
...
def extract_tarball(tar, destdir):
    ...
    if hasattr(tar, 'read'):
        tar = tar.name
    subprocess.check_call(['tar', 'xjof', tar, '-C', destdir], stdout=open('/dev/null', 'w'),
            preexec_fn=lambda:
                        signal.signal(signal.SIGPIPE, signal.SIG_DFL))
...
def main():
    ...
    mh = os.path.join(destdir, 'bin/calibre-mount-helper')
    if os.geteuid() == 0:
        os.chown(mh, 0, 0)
        os.chmod(mh,
